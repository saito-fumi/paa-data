...
function userEventBeforeLoad(type, form, request) {
    ...
    if (nlapiGetContext().getExecutionContext() == "userinterface" && (type == "view" || type == "edit")) {
        var intRecordTypeID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ue_applies_to_record_type2');
        var intHTMLTemplateRecordID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ue_html_template_record2');
        var strExistFileFieldID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ue_exist_file_id2');
        var intTrxnForm = '';
        var intSubsidiaryID = '';
        var intReturnTypeID = '';
...
function userEventAfterSubmit(type) {
    ...
    try {
        if (nlapiGetContext().getExecutionContext() == 'userinterface') {
            var scriptParamTransactionId = nlapiGetContext().getSetting('SCRIPT', 'custscript_ue_html_template_record2');// Function.getDefaultHTMLTemplate(nlapiGetRecordType());
