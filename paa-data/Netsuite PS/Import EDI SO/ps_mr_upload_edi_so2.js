 ...
 * 1.00		2022/05/11				Keito Imai		Initial Version
 * 
 */
...
define([
	...
	function(plugin, record, runtime, email, search, url, format, file, error){
		...
		function getInputData(){
			...
			try{
				var soInfo = JSON.parse(runtime.getCurrentScript().getParameter({name: 'custscript_ns_edi_so_info2'}));
		...
		function reduce(context){
			...
			try{
				...
				const cf = runtime.getCurrentScript().getParameter({name: 'custscript_ns_edi_so_form2'});
				const otherrefnum = headerInfo.e;
				var tranDate = new Date(headerInfo.d);
				tranDate.setDate(tranDate.getDate() + 1);
				var shipDate = new Date(headerInfo.s.split('/')[0], headerInfo.s.split('/')[1] - 1, headerInfo.s.split('/')[2]);
				//shipDate.setDate(shipDate.getDate() + 1);
				const entity = headerInfo.c;
				const address = headerInfo.a;
				const memo1 = headerInfo.m;
				const memo2 = headerInfo.n;
				const mainDept = runtime.getCurrentScript().getParameter({name: 'custscript_ns_edi_so_dept2'});
				const lineDept = runtime.getCurrentScript().getParameter({name: 'custscript_ns_edi_so_line_dept2'});
				const location = runtime.getCurrentScript().getParameter({name: 'custscript_ns_edi_so_location2'});
		...
		function summarize(context) {
			...
			try{
				...
				email.send({
					...
					subject: '注文アップロード処理（第一貨物）が完了しました。',
					body: mailBody
				});
			}catch(e){
				log.error('e', e);
				email.send({
					author: runtime.getCurrentUser().id,
					recipients: runtime.getCurrentUser().id,
					subject: '注文アップロード処理（第一貨物）でエラーが発生しました。',
