		...
		for(var i = 0; i < context.currentRecord.getLineCount({sublistId: SUBLIST_ID}); i++){
			...
			if(cr.getSublistValue({sublistId: SUBLIST_ID, fieldId: 'custpage_to_be_processed', line: i})){
				...
				try{
					...
					successMessages.push('締め請求書バッチレコード内部ID:'+idsInternalId+' -> 削除成功しました。');
					redirectFlg = true;
				}catch(e){
					//alert('catch:'+e);
					console.log(e);
					...
					if(e.name == "THIS_RECORD_CANNOT_BE_DELETED_BECAUSE_IT_HAS_DEPENDENT_RECORDS"){
						errorMessages.push('締め請求書バッチレコード内部ID:'+idsInternalId+' -> 先に締め請求書トランザクションの削除を行ってください。');
					}else if(e.name == "RCRD_DSNT_EXIST"){
						errorMessages.push('締め請求書バッチレコード内部ID:'+idsInternalId+' -> レコードが既に存在しません。');
					}else{
						errorMessages.push('締め請求書バッチレコード内部ID:'+idsInternalId+' -> 予期しない原因により削除できませんでした。');
					}
