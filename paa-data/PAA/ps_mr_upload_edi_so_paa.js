 ...
 * 1.00		2019/2/5				Keito Imai		Initial Version									
 * 																	
 */																	
...
define([																	
	...
	function(plugin, record, runtime, email, search, url, format, file, error){																
		...
		function getInputData(){															
			...
			try{
			    var arrdata =runtime.getCurrentScript().getParameter({name: 'custscript_so_csv_json'});
			    log.debug('params', arrdata);
				var soInfo = JSON.parse(arrdata);													
		...
		function reduce(context){															
			...
			try{														
				...
				const cf = runtime.getCurrentScript().getParameter({name: 'custscript_so_csv_form'});		
				...
				const mainDept = runtime.getCurrentScript().getParameter({name: 'custscript_so_csv_dept'});													
				const lineDept = runtime.getCurrentScript().getParameter({name: 'custscript_so_csv_line_dept'});													
				const location = runtime.getCurrentScript().getParameter({name: 'custscript_so_csv_location'});													
...
//				soRecord.setValue({fieldId: 'custbody_ns_wms_memo2', value: memo2, ignoreFieldChange: true});													
		...
		function summarize(context) {															
			...
			try{														
				...
				log.debug('successArray', successArray);													
				log.debug('failedArray', failedArray);													
				...
				if(successArray.length > 0){													
					...
					log.debug('formulaText', formulaText);												
					...
					.run();												
					log.debug('soSearchResultSet', soSearchResultSet);												
					//検索実行結果をループ												
